...
export const CATEGORY_HUBS: Record<string, ProgrammaticPageData> = {
  'wordpress-themes': {
    ...
    intro: 'Looking to buy a WordPress theme that delivers 100/100 Core Web Vitals without the crippling bloat of multi-gigabyte page builders? Wefik.world provides curated, agency-grade WordPress themes engineered in-house. Every theme is built with semantic HTML5, native Gutenberg Full Site Editing (FSE) block patterns, and modern responsive CSS. Whether you need a theme for a client agency portfolio, a modern SaaS showcase, or a high-converting local business site, our products provide commercial rights, instant crypto-verified license keys, and lifetime updates.',
    siblingLinks: [
  ...
  'wordpress-plugins': {
    ...
    intro: 'Most commercial WordPress plugins slow down your admin dashboard with redundant database queries, aggressive upsell notices, and unminified assets. Wefik.world plugins are engineered with single-vendor discipline: strict object-oriented PHP 8.2 standards, automated database indexing, and zero administrative bloat. From static page caching and WebP asset conversion to login hardening and automated JSON-LD schema generation, our plugins help agencies build bulletproof client sites without monthly subscription fatigue.',
    siblingLinks: [
  ...
  'html-templates': {
    ...
    intro: 'When you need the absolute maximum loading speed and zero CMS dependencies, static HTML remains the undisputed king of web development. Wefik.world delivers modern HTML templates crafted with Tailwind CSS and modern CSS grid architectures. Perfect for SaaS waiting lists, agency portfolios, marketing funnels, and enterprise admin portals, each template comes with modular components, pristine semantic markup, and pre-configured SEO tags ready for instant deployment on Vercel, Netlify, or Cloudflare Pages.',
    siblingLinks: [
      { label: 'WordPress Themes', url: '/wordpress-themes' },
  ...
  'code-snippets': {
    ...
    intro: 'Stop wasting hours reinventing common utilities, custom authentication flows, or complex CSS animations. Wefik.world code snippets provide tested, modular code blocks for modern developers. From custom WordPress REST API handlers and Razorpay webhook verifiers to Tailwind micro-animations and headless CMS connectors, our snippet catalog gives agencies and indie developers battle-tested building blocks that save dozens of engineering hours per project.',
    siblingLinks: [
      { label: 'WordPress Plugins', url: '/wordpress-plugins' },
      { label: 'HTML Templates', url: '/html-templates' },
